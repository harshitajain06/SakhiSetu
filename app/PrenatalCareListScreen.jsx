...
import { prenatalCareData } from './data/prenatalCareData';

export default function PrenatalCareListScreen() {
  ...
  const getTranslatedTitle = (itemId) => {
    try {
      ...
      const translatedTitle = currentTranslations?.prenatalCare?.prenatalCareItems?.[itemKey]?.title;
      return translatedTitle || prenatalCareData.find(i => i.id === itemId)?.title || '';
    } catch (error) {
      console.log('Translation error:', error);
      return prenatalCareData.find(i => i.id === itemId)?.title || '';
    }
  };
  ...
  const renderItemCard = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('PrenatalCareDetail', { item: item })}
        activeOpacity={0.7}
      >
